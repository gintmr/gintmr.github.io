...
function populatePapers(items, id) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    abstractP.className = "timeline-text";
    abstractP.innerHTML = items[i].abstract;

    // 组装标签容器
    const divTimelineLabel = document.createElement("div");
    divTimelineLabel.className = "timeline-label";
    divTimelineLabel.appendChild(h2TimelineLabel);
    divTimelineLabel.appendChild(spanTimelineSublabel);
    divTimelineLabel.appendChild(abstractP);

    // 创建图标
    const iFa = document.createElement("i");
    iFa.className = "fa fa-file-text"; // 使用文档图标

    const divTimelineIcon = document.createElement("div");
    divTimelineIcon.className = "timeline-icon color-2";
    divTimelineIcon.appendChild(iFa);

    // 组装时间线条目
    const divTimelineEntryInner = document.createElement("div");
    divTimelineEntryInner.className = "timeline-entry-inner";
    divTimelineEntryInner.appendChild(divTimelineIcon);
    divTimelineEntryInner.appendChild(divTimelineLabel);

    const article = document.createElement("article");
    article.className = "timeline-entry animate-box";
    article.appendChild(divTimelineEntryInner);

    mainContainer.appendChild(article);
  }

  // 添加时间线结束标记
  const endMarker = document.createElement("article");
  endMarker.className = "timeline-entry begin animate-box";
  endMarker.innerHTML = `
