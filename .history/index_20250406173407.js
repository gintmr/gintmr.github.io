...
function populatePapers(items, id) {
  const papersContainer = document.getElementById(id);
  
  items.forEach(paper => {
    const paperDiv = document.createElement("div");
    paperDiv.className = "col-md-12 animate-box";
    
    paperDiv.innerHTML = `
      <div class="paper-item">
        <h3><a href="${paper.link}" target="_blank">${paper.title}</a></h3>
        <p class="authors">${paper.authors}</p>
        <p class="venue">${paper.conference}, ${paper.year}</p>
...
populateTrekking(trekking, "trekking");
