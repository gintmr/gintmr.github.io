...
SeaTree: Tracing Marine Creatures through Taxonomic Tree is **Submitted to AAAI 2027** and retains its contributing-author role. Its author list is Yiwei Chen, Zheng Ziqiang, Xinrui Wu, Sai-Kit Yeung, with each name linked to the corresponding OpenReview profile supplied by the author through the entry's `authorLinks` object. No equal-contribution markers or paper URL are inferred for SeaTree.
