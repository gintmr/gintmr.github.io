...
The CV publication section takes precedence over older internship submission text: BudgetThinker, MaskGuide, and ImageTime are accepted; HiReT and the action-aware clinical world-model manuscript are submitted to ICLR 2027. HiReT replaces the previous HiCausal entry. ParaThinker is accepted by EMNLP 2026, as confirmed by the author. The author supplied complete author lists for BudgetThinker and ParaThinker, the updated ParaThinker title, and the OCG entry submitted to NeurIPS 2026 Workshop BeNTo with its alphaXiv link. BudgetThinker marks Hao Wen and Xinrui Wu as equal contributors. OCG appears first in the Submitted section. Seatree retains its contributing-author role and ECCV 2026 submission status from the original homepage; no full author list or paper URL has been added for it.
